...
1) Использовать защищённые протоколы. При работе в сети лучше использовать протоколы с шифрованием, таким как HTTPS, SSH и FTPS/SFTP.
2) Не подключаться к публичным сетям Wi-Fi для конфиденциальных транзакций.
3) Использовать VPN.
4) Следить за подозрительной активностью. Если веб-страницы ведут себя странно, внезапно обрывается соединение или появляются другие аномалии, это может быть признаком взлома.
Для сетевых администраторов:
1) Использовать системы обнаружения вторжений.
2) Внедрять Internet Protocol Security.
